import os, re, datetime as dt, requests, sys
...
def fetch(url, name):
    ...
    if "pdf" not in ctype:
        print(f"[warn] content-type not pdf: {ctype}")
...
def append_csv(df, path):
    if df.empty:
        print(f"[info] {path}: no rows to append")
        return
    if os.path.exists(path):
        old = pd.read_csv(path)
        df = pd.concat([old, df], ignore_index=True).drop_duplicates()
    df.to_csv(path, index=False)
    print(f"[ok] wrote {path} ({len(df)} rows total)")
...
def main():
    ...
    if not prices.empty:
        ...
        if not latest.empty:
            ax = latest.groupby('item')['mid'].mean().nlargest(10).plot(kind="barh", title="TOP10 mid prices")
            import matplotlib.pyplot as plt
            plt.tight_layout()
            plt.savefig(f"reports/{TODAY}/price_top10.png")
            print("[ok] saved reports/{}/price_top10.png".format(TODAY))
